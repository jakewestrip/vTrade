...
{
    public interface IBuyShareCommand
    {
        Task<bool> Execute(string Ticker, int NumShares, int UserId);
    }

    public class BuyShareCommand : IBuyShareCommand
    {
        private readonly IAmazonDynamoDB _amazonDynamoDB;

        public BuyShareCommand(IAmazonDynamoDB amazonDynamoDB)
        {
        ...
        public async Task<bool> Execute(string Ticker, int NumShares, int UserId)
        {
            try
            {
                ...
                if(portfolio.OwnedShares.Any(x => x.Ticker == Ticker))
                {
                    portfolio.OwnedShares.First(x => x.Ticker == Ticker).Shares += NumShares;
                }
                else
                {
                    portfolio.OwnedShares.Add(new OwnedShare() { Ticker = Ticker, Shares = NumShares });
                }
